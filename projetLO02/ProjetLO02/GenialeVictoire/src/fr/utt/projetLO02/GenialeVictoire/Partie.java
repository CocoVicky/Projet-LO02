...
public class Partie {
 ...
 documentation de la classe Joueur.
	...
	/**
	 * La liste des cartes de tous les jests confondus des joueurs.
	 * @see Partie#getListeJest()
	 * @see Partie#setListeJest(ArrayList)
	 */
	private ArrayList<Carte> listeJest; 
	
	/**
	 * La liste des cartes de récompense qui seront attribuées aux joueurs qui respectent les conditions des orange band.
	 * @see Partie#getListeTrophees()
	 * @see Partie#setListeTrophees(ArrayList)
	 */
	private ArrayList<Carte> listeTrophees;
	
	/**
	 * Le joueur choisit au début de la partie de s'il veut une partie normale ou une variante de la partie.
	 * @see Partie#getMode()
	 * @see Partie#setMode(int)
	 */
	private int mode;
	
	/**
	 * Le joueur choisit également au début de la partie s'il veut une version de jeu avec ou sans extension.
	 * @see Partie#isExtension()
	 * @see Partie#setExtension(boolean)
	 */
...
initialisé à false.
	...
	public void configurer() {
		...
		System.out.println("Mode 1 : partie normale ");
		System.out.println("Mode 2 : partie variante : toutes les cartes sont comptées positivement, le joker ne vaut rien.");
		System.out.println("Quelle mode préférez vous?");
		int mode = verifier(1 , 2);
		setMode(mode);
		System.out.println("Mode 3 : partie sans extension ");
		System.out.println("Mode 4 : partie avec extension de carte");
		System.out.println("Quelle mode préférez vous?");
		int extension = verifier(3 , 4);
	...
	/**
	 * Distribue deux cartes du stack à chaque joueur.
	 * C'est cette méthode qui est appelée à chaque round.
	 * @see Partie#stack
	 */
	 ...
	 * @return les cartes représentant les trophées.
	 * @see Partie#listeTrophees
	 */
	public String presenterTrophees(){
		if (this.listeJoueurs.size() == 3 && this.extension == false) {
			this.banque.getJeu().remove();
			for (int i = 0; i < 2; i++) {
				Carte trophee = this.banque.donnerCarte(); 
				while ((trophee.getValue() == Valeur.Joker) || (trophee.getValue() == Valeur.Bonus)) {
					this.banque.melanger();
				}
				listeTrophees.add(trophee);	
			}
		}else if(this.listeJoueurs.size() == 4 && this.extension == false) {
			this.banque.getJeu().remove();
			Carte trophee  = this.banque.donnerCarte();
			while ((trophee.getValue() == Valeur.Joker) || trophee.getValue() == Valeur.Bonus) {
				this.banque.melanger(); 
			...
			listeTrophees.add(trophee);
		}else if(this.listeJoueurs.size() == 3 && this.extension == true){
			for (int i = 0; i < 3; i++) {
				Carte trophee = this.banque.donnerCarte(); 
				while ((trophee.getValue() == Valeur.Joker) || (trophee.getValue() == Valeur.Bonus)) {
					this.banque.melanger();
		...
		}else if(this.listeJoueurs.size() == 4 && this.extension == true) {
			for (int i = 0; i < 2; i++) {
				Carte trophee = this.banque.donnerCarte(); 
				while ((trophee.getValue() == Valeur.Joker) || (trophee.getValue() == Valeur.Bonus)) {
					this.banque.melanger();
	...
	/**
	 * Ajoute aux jests de chaque joueur qui respecte les conditions énoncées sur les cartes trophées.
	 * @see Partie#listeTrophees
	 * @throws ConcurrentModificationException
	 */
	public void attribuerTrophees () throws ConcurrentModificationException {
...
la liste de cartes entrée en paramètre.
	...
	/**
	 * <p>
	 * Parcourt la liste des points des joueurs et désigne le gagnant, celui à qui appartient
le plus grand nombre de points.
	 * </p>
	 * @param listePoints
	 * @return le joueur gagnant.
	 * @see Partie#listePoints
	 */
	...
	public Joueur designerGagnant(ArrayList<Integer> listePoints) {
		...
		int etape = 0 ;
		int icherch, inter;
		for (etape = 0 ; etape <= listePoints.size() - 1; etape++) {
			int posMax = etape;
	...
	/**
	 * <p>
	 * Vérifie que le numéro de joueur victime que le prioritaire a choisi respecte les
différentes conditions fixées par les règles du jeu.
	 * </p>
	 * @param numJoueur
	 * @param ontJoue
	 * @param prioritaire
	 * @return si oui ou non le choix du prioritaire est valide.
	 */
	public boolean verifierValiditeChoix(int numJoueur , ArrayList<Joueur> ontJoue , Joueur prioritaire ) {
		...
		if (numJoueur == prioritaire.getIndiceJoueur()) {
			...
			while ((prioritaire != null) && prioritaire.isaJoue() == false){
				...
				do { 
					if(prioritaire.getNiveau() == 0) {
						...
						numJoueur = ((JoueurVirtuel)prioritaire).getStrategie().choisirJoueur(1, listeJoueurs.size(), listeOffres, listeJoueurs , ontJoue , this);//choisir un joueur cible pour un joueur virtuel
					...
					System.out.println("peut = " + verifierValiditeChoix(numJoueur, ontJoue, prioritaire));
				
				}while (verifierValiditeChoix(numJoueur, ontJoue, prioritaire) == false);
	...
	/**
	 * <p>
	 * Compte  le nombre de points correspondant à chaque joueur,
 et affiche les points de chaque joueur.
	 * </p>
	 * @param listePoints
	 */
	public void comptabiliser(ArrayList<Integer> listePoints) {
		CompteurPointsVisitor compteurVisitor = new CompteurPointsVisitor();
		Iterator<Joueur> itPts = listeJoueurs.iterator();
		while (itPts.hasNext()) {
			Joueur j = itPts.next();
			compteurVisitor.visit(j.getJest() , this);	
	...
	public ArrayList<Carte> getListeJest() {
		return listeJest;
	}

	public void setListeJest(ArrayList<Carte> listeJest) {
		this.listeJest = listeJest;
	}

	public ArrayList<Carte> getListeTrophees() {
		return listeTrophees;
	}

	public void setListeTrophees(ArrayList<Carte> listeTrophees) {
		this.listeTrophees = listeTrophees;
	}
